...
{
    /**
     * @brief Loads all command definitions compiled into the binary.
     *
     * This method initializes the command definitions from a precompiled header file
     * generated by the CLI precompiler tool. The header path is specified by the
     * `CLIENGINE_PRECOMPILED_HEADER_PATH` macro and included at compile time.
     *
     * This function must be called before any calls to `execute()`.
     *
     * Since this method is marked `noexcept`, it does not throw exceptions.
     * Instead, it logs any errors or warnings encountered during loading, such as:
     * - Missing or undefined `CLIENGINE_PRECOMPILED_HEADER_PATH` macro
     * - Empty or missing precompiled command definitions
     *
     * It is the caller's responsibility to ensure the precompiled commands
     * are properly loaded before execution.
     *
     * @see CLIENGINE_PRECOMPILED_HEADER_PATH
     * @see CommandDef
     */
    void loadPredefinedCommands() noexcept;
   ...
   private:
    std::unordered_map<std::string, CommandDef> commandDefs;     // Populated map of commandDefs
